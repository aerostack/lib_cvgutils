...
int main(int argc,char **argv) {
    std::cout << "Compile test" << cvg_int_to_string(10) << std::endl;
...
//        pi = &j;
    ...
    // Pruebas con XMLFileReader
    {
        ...
        {
            static const std::string vr = "ekf_state_estimator_config:module_frequency";
            result_double = my_xml_reader.readDoubleValue(vr);
            std::cout << "result: " << result_double << std::endl;
        }
        {
            static const std::string vr = "ekf_state_estimator_config:state_model:number_of_states";
            result_int = my_xml_reader.readIntValue(vr);
            std::cout << "result: " << result_int << std::endl;
        }
        {
//            static const std::string vr = "ekf_state_estimator_config:state_model:delete_this_parameter";
//            try {
...
//            }   std::cout << "}" << std::endl;
        ...
        std::size_t pos;
        double my_d = cvg_string_to_double(prueba_varios_doubles);
        std::cout << "my_d:" << my_d << " pos:" << pos << std::endl;
        prueba_varios_doubles = prueba_varios_doubles.substr( pos);
        my_d = cvg_string_to_double(prueba_varios_doubles);
        std::cout << "my_d:" << my_d << " pos:" << pos  << std::endl;
        prueba_varios_doubles = prueba_varios_doubles.substr( pos);
        my_d = cvg_string_to_double(prueba_varios_doubles);
        std::cout << "my_d:" << my_d << " pos:" << pos  << std::endl;
...
//    std::cout << "(int) = (bool,false): " << a_b << std::endl;
    ...
    {
        int idDrone = 0;
        XMLFileReader my_xml_reader(std::string(std::getenv("DRONE_STACK"))+"/configs/drone"+cvg_int_to_string(idDrone)+"/trajectory_controller_config.xml");
...
//        std::cout << "(my_enum < myPrivateEnum::GREEN):" << (my_enum < myPrivateEnum::GREEN) << std::endl;
    ...
    {
        //JL TODO -> Fix!
        XMLFileReader my_xml_reader(std::string("/home/jespestana/workspace/ros/quadrotor/stack/droneSimulator/configs/pelican_like_model.xml"));
    }

    {
        std::cout << cvg_int_to_string(50) << std::endl;
        std::cout << cvg_string_to_double(" 50.5E-1  ") << std::endl;
        std::cout << cvg_string_to_int("  50  ") << std::endl;
    }

    {
        srand( time(0) );
        std::cout << "cvg_normal_distribution check" << std::endl;
        for (int i=0; i<10; i++) {
            std::cout << cvg_normal_distribution( 10.0, 2.0) << std::endl;
        }
        std::cout << "cvg_uniform_random check" << std::endl;
        for (int i=0; i<10; i++) {
            std::cout << cvg_uniform_random( -4.0, -3.0) << std::endl;
        }
    }

    {
        Timer cvg_timer;
        usleep( 100000 );
        cvg_timer.restart(false);
        usleep( 200000 );
        std::cout << "cvg_timer.getElapsedSeconds():" << cvg_timer.getElapsedSeconds() << std::endl;
        usleep( 200000 );
        std::cout << "cvg_timer.getElapsedSeconds():" << cvg_timer.getElapsedSeconds() << std::endl;
    }
